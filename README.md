...
- Final measured throughput after the first write-path optimization:
  - With Linux/FSBL already running: Write ~`479 MiB/s`, Read ~`455 MiB/s`.
  - With JTAG blank boot (no Linux): Write ~`509 MiB/s`, Read ~`455 MiB/s`.
  - The write speed gain comes from eliminating Linux DDR background traffic.
...
- PL reset: `rtl/pl_por.v`, self-contained ~5 ms power-on reset from `sys_clk`.
...
```text
build_pl_ps_ddr_mem_test.tcl: pl_clk_mhz, pl_clk_hz (used by tester and pl_por)
...
flowchart TB
    ...
    class Tester tester
    class PS ps
    class Clk,SysClk,Aclk,PlPor clock
...
```mermaid
flowchart LR
    SysClk[sys_clk 200 MHz] --> PlPor[pl_por_0<br/>~5 ms power-on reset]
...
10. Create external sys_clk port.
11. Connect sys_clk to tester, SmartConnect, and PS HP0 AXI clock.
12. Connect pl_por_0/rstn to tester and SmartConnect aresetn.
13. Connect tester M_AXI through SmartConnect to PS S_AXI_HP0_FPD.
14. Export UART, DDR, and FIXED_IO ports.
15. Validate and save block design.
16. Confirm `ddr_tester_0/M_AXI` has `HP0_DDR_HIGH` assigned.
17. Generate HDL wrapper.
18. Run synthesis.
19. Run implementation.
20. Write bitstream.
21. Export hardware platform XSA (for FSBL generation).
```
...
bandwidth, so write throughput is higher than the Linux-running measurements
below.
...
write_cycles : 1544
read_cycles  : 1714
write_time_s : 0.000007720
read_time_s  : 0.000008570
write_mibps  : 505.991
read_mibps   : 455.805
error_count  : 0
result       : PASS
...
```text
write_cycles : 6286458
read_cycles  : 7039425
write_time_s : 0.031432290
read_time_s  : 0.035197125
write_mibps  : 509.031
read_mibps   : 454.583
error_count  : 0
result       : PASS
...
write_cycles : 1534
read_cycles  : 1708
write_time_s : 0.000007670
read_time_s  : 0.000008540
write_mibps  : 509.289
read_mibps   : 457.406
error_count  : 0
result       : PASS
...
write_cycles : 49114
read_cycles  : 54995
write_time_s : 0.000245570
read_time_s  : 0.000274975
write_mibps  : 509.020
read_mibps   : 454.587
error_count  : 0
result       : PASS
...
```text
write_cycles : 6286459
read_cycles  : 7039456
write_time_s : 0.031432295
read_time_s  : 0.035197280
write_mibps  : 509.031
read_mibps   : 454.581
error_count  : 0
result       : PASS
...
```text
write_cycles : 402333347
read_cycles  : 450522727
write_time_s : 2.011666735
read_time_s  : 2.252613635
write_mibps  : 509.031
read_mibps   : 454.583
error_count  : 0
result       : PASS
...
### Linux-Running Results (Historical Reference)

These results were measured before the blank-boot flow was added, with Linux
already running on the PS. Linux background DDR traffic reduces write
throughput by about 6%. Read throughput is unchanged because the read path is
limited by the single-outstanding read engine, not by DDR arbitration.
...
improved write throughput further. Eliminating Linux background DDR traffic (via
JTAG blank boot) improved write throughput by another ~6%. Read throughput is
mainly limited by the simple single-outstanding read engine and is unaffected by
PS software load.
...
200 MHz E12 after opt, Linux 1525.879 MiB/s 479.090 MiB/s    31.40%       454.584 MiB/s   29.79%
...
with that traffic. The JTAG blank boot flow eliminates this contention: with no
PS software running, write throughput rises from ~479 MiB/s to ~509 MiB/s. Read
throughput is unaffected because it is limited by the single-outstanding read
engine, not by DDR arbitration.
...
The final measured result with JTAG blank boot (no Linux) is:

```text
write: 509.031 MiB/s, about 33.36% of the 64-bit 200 MHz AXI ideal peak
read : 454.583 MiB/s, about 29.79% of the 64-bit 200 MHz AXI ideal peak
```

With Linux running, write drops to 479.090 MiB/s (31.40%) due to PS background
DDR traffic. Read is unchanged at 454.584 MiB/s (29.79%).
...
- PL reset is generated by `rtl/pl_por.v` from `sys_clk`, not from PS
  `pl_resetn0`. The tester and SmartConnect are released from reset ~5 ms after
  bitstream configuration, independent of PS firmware state. This makes the UART
  debug path usable immediately after programming, even on a blank PS.
...
generation, UART debug capture) was added with glm-5.2.
